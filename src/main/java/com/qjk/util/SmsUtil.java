...
public class SmsUtil {
	...
	public static Sms sendSms(Sms sms) throws UnsupportedEncodingException {
		...
		url_ = url_.replace("{body}", URLEncoder.encode(SmsUtil.prefix+sms.getContent(), charset));
		
		try {
			
			String text = "POST".equals(method) ? CURL.post(new URL(url_)).exec() : CURL.get(new URL(url_)).exec();
